...
void oledStartForGraph() {
	int temp_graph = temperature1 * 10.0;
	for (int cnt = 0; cnt < 120; cnt++) {
		temp_in[cnt] = temp_graph;
	}
...
void oledOutGraphDisplay() {
  String time_ntp = GetTime();
  int count, temp_min = 1000, temp_max = 0;;
  int temp_rez, temp_dec, temp_inc;

  if (readTempInterval >= Display_out_temp || (int)temp_in[119] != (int)(temperature1 * 10)) {
